# E5T4 — Alert message formatting and delivery

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 CWTR

Implement the formatting and delivery of alert messages. Ensure messages include coin, old price, new price, absolute and percent change, and the rule that triggered the alert. Write tests to tests/specs/E5T4.json and append the command to tests/commands.json.
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E5T4.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/E5T4.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
