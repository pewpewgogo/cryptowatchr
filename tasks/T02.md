...
Implement `/start`: a welcome message and the bot's main menu (inline keyboard) routing to its top-level features. Write the `/start` dialog spec to your OWN `tests/specs/T02.json` (a BotSpec array) and declare the command in your OWN `tests/commands/T02.json` (a JSON string array, e.g. `["/start"]`) — never a shared file.
